...
export function KanbanBoard() {
  ...
  function getDraggingTaskData(taskId: UniqueIdentifier, columnId: ColumnId | null) {
    if (!columnId) {
    ...
    >
      <IonContent>
        <BoardContainer>
          ...
          <IonGrid fixed={true}>
            <IonRow style={{ maxHeight: '100vh' }}>
              {columns.map((col) => (
                <IonCol key={col.id} className="ion-margin" style={{ maxHeight: '100vh' }}>
                  <BoardColumn column={col} tasks={tasks.filter((task) => task.columnId === col.id)} />
                </IonCol>
              ))}
            </IonRow>
          </IonGrid>
